...
rects_t &find_rects(uint8_t *img, float min_magnitude, apriltag_detect_visualize_flag visualize_flag) {
    staticBuffer.reset();
#if (apriltag_benchmark)
    int32_t t0 = rt_tick_get();
#endif
    ...
    auto &clusters = *gradient_clusters(threshim, img, min_magnitude);
    if (visualize_flag == apriltag_detect_visualize_flag::clusters) show_clustersImg(img, clusters);
