...
static inline void drawCircleUtil(int xc, int yc, int x, int y, drawPoint_t &drawPoint) {
...
void drawCircle(int xc, int yc, int r, drawPoint_t drawPoint) {
    int x = 0, y = r;
    int d = 3 - (r << 1);
    drawCircleUtil(xc, yc, x, y, drawPoint);
    while (y >= x) {
        ++x;
        if (d > 0) --y, d += ((x - y) << 2) + 10;
        else
